...
class disco_fireball : ScriptBaseEntity {
	...
	void DetoThink(){
		if(self !is null && g_EntityFuncs.IsValidEntity( self.edict() )){
			g_EntityFuncs.CreateExplosion(self.pev.origin, Vector(0,0,0), null, 39, true);
...
class disco_disconator : ScriptBaseAnimating {
	...
	void ThinkThrowBombRight2(){
		...
		if(pHealth > 90.0f){
			...
			power = (sqrt(g_disco_aim_cross.targetPos.x*g_disco_aim_cross.targetPos.x+g_disco_aim_cross.targetPos.y*g_disco_aim_cross.targetPos.y) -100.0f) / 1000.0f;
			if(power < 0.0f) power = 0.0f;
			if(power > 1.0f) power = 1.0f;
		}
		
		pEntity.pev.velocity.x = cos(self.pev.angles.y/180.0f*3.14159265358979323846f) * (60.0f + 500.0f * power);
		pEntity.pev.velocity.y = sin(self.pev.angles.y/180.0f*3.14159265358979323846f) * (60.0f + 500.0f * power);
...
class disco_disconator2 : ScriptBaseAnimating {
	...
	void ThinkThrowBombRight2(){
		...
		float power = (sqrt(g_disco_aim_cross.targetPos.x*g_disco_aim_cross.targetPos.x+g_disco_aim_cross.targetPos.y*g_disco_aim_cross.targetPos.y) -100.0f) / 1000.0f;
		if(power < 0.0f) power = 0.0f;
		if(power > 1.0f) power = 1.0f;
...
class disco_disconator4 : ScriptBaseAnimating {
	...
	void ThinkThrowBombRight2(){
		...
		pEntity.pev.velocity.x = sin(self.pev.angles.y/180.0f*3.14159265358979323846f) * (60.0f + 500.0f * power);
		pEntity.pev.velocity.y = -cos(self.pev.angles.y/180.0f*3.14159265358979323846f) * (60.0f + 500.0f * power);
		pEntity.pev.velocity.z = 140.0f + 140.0f * power;
...
void RegisterDiscoDisconatorEasyCustomEntity() {
